...
from agbs.abstract_domains.deeppoly_domain import IntervalLattice
...
class ProductState(State):
    ...
    def affine(self, left: List[PyVar], right: List[PyTexpr1]) -> 'ProductState':
        ...
        for lhs in left:
            name = str(lhs)
            self._share_bounds(name)
            bounds = self.bounds[name]
            lower, upper = bounds.lower, bounds.upper
            if lower < 0 and 0 < upper:

                inf, sup = dict(), dict()
                expr = [domain.get_expressions(name) for domain in self._domains]
                for e in expr:
                    i, s = e[0], e[1]
                    for key, value in i.items():
                        inf[key] = inf.get(key, 0) + abs(value)
                    for key, value in s.items():
                        sup[key] = sup.get(key, 0) + abs(value)
                self.expressions[name] = (inf, sup)
                self.polarities[name] = (lower + upper) / (upper - lower)
