...
from agbs.core.expressions import Lyra2APRON, NegationFreeExpression, VariableIdentifier, Expression, \
    BinaryComparisonOperation, BinaryBooleanOperation, Literal, UnaryArithmeticOperation
from agbs.abstract_domains.deeppoly_domain import IntervalLattice, texpr_to_dict, evaluate
...
class NeurifyState(State, BoundsDomain):
    ...
    def __init__(self, inputs: Set[VariableIdentifier], precursory: State = None):
        ...
        self.bounds = {input: (IntervalLattice(-inf, inf), IntervalLattice(-inf, inf))
                       for input in self.inputs}

        # the free coeff is indicated by '_' in the dictionary self.poly
        self.poly = {input: ({'_': -inf}, {'_': inf}) for input in self.inputs}
    ...
    def __assume_helper(self, feature, lower, upper):
        self.bounds[feature.name] = (
            self.bounds[feature.name][LOW].meet(IntervalLattice(lower, lower)),
            self.bounds[feature.name][UP].meet(IntervalLattice(upper, upper))
        )
    ...
    def assume(self, condition, manager: PyManager = None, bwd: bool = False) -> 'NeurifyState':
        ...
        if isinstance(condition, list):
            ...
            if condition.operator == BinaryBooleanOperation.Operator.Or:
                ...
                lowlow = self.bounds[condition.left.right.name][LOW].lower
                lowup = self.bounds[condition.left.right.name][LOW].upper
                uplow = self.bounds[condition.left.right.name][UP].lower
                upup = self.bounds[condition.left.right.name][UP].upper
                self.bounds[condition.left.right.name] = (
                    IntervalLattice(max(lowlow, lower), max(lowup, lower)),
                    IntervalLattice(min(uplow, upper), min(upup, upper))
                )
                self.poly[condition.left.right.name] = (
                    {'_': self.bounds[condition.left.right.name][LOW].lower},
                    {'_': self.bounds[condition.left.right.name][UP].upper}
                )
                ext = IntervalLattice(
                    self.bounds[condition.left.right.name][LOW].lower,
                    self.bounds[condition.left.right.name][UP].upper
                )
                if ext.is_bottom():
                    return self.bottom()
                return self
