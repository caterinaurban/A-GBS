        ...
        if self.is_bottom():
            ...
            for chosen in outcomes:
                ...
                for discarded in remaining:
                    ...
                    exp = dict()
                    exp[chosen.name] = 1
                    exp[discarded.name] = -1
                    # print(exp)
                    while any(variable in exp and variable not in self.inputs for variable in self.poly):
                        # print(exp)
                        for variable in self.poly:
                            if variable in exp and variable not in self.inputs:  # should be replaced
                                coeff = exp[variable]
                                if coeff > 0:
                                    replacement = self.poly[variable][0]
                                elif coeff < 0:
                                ...
                                for var, val in replacement.items():
                                    if var in exp:
                                        ...
                                        exp[var] = coeff * val
                    value = evaluate(exp, self.bounds)
                    # print(value)
                    # chosen <= discarded
                    if lower <= upper: #value.upper <= 0:  #
                        unique = False
                        break
                if unique:
                    found = chosen
