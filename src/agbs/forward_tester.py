...
from agbs.core.cfg import Activation

from agbs.engine.forward_runner import ForwardAnalysis
from agbs.engine.bias_analysis import AbstractDomain
from agbs.main import checker

spec = ''
nn = 'tests/M-FNN-100-BN-small.py'
if len(sys.argv) > 1:
    domain = checker(sys.argv[1])
else:
    domain = AbstractDomain.SYMBOLIC3 #ALL  #SYMBOLIC3   #BOXES2
...
# }
start = time.time()
r = b.main(nn)      # , forced_active_names=forced_active, forced_inactive_names=forced_inactive)
end = time.time()
print('Analysis Time: {}s'.format(end - start))
