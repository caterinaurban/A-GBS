...
from agbs.abstract_domains.deeppoly_domainA import DeepPolyStateA
from agbs.abstract_domains.deeppoly_domainB import DeepPolyStateB
...
from agbs.engine.runner import Runner
...
from agbs.abstract_domains.interval1_domain import Box1State
from agbs.abstract_domains.interval2_domain import Box2State
from agbs.abstract_domains.symbolic1_domain import Symbolic1State
from agbs.abstract_domains.symbolic2_domain import Symbolic2State
...
from agbs.frontend.cfg_generator import ast_to_cfg
from agbs.engine.forward import ForwardInterpreter, ActivationPatternForwardSemantics
...
class ForwardAnalysis(Runner):
    ...
    def state(self):
        ...
        elif self.domain == AbstractDomain.DEEPPOLY_NEURIFY_SYMBOLIC3:
            state = ProductState(inputs, [DeepPolyState(inputs), NeurifyState(inputs), Symbolic3State(inputs)])
        else:
            assert self.domain == AbstractDomain.ALL
            assert self.domain == AbstractDomain.ALL
            state = ProductState(inputs, [
                Symbolic3State(inputs),
                DeepPolyState(inputs),
                DeepPolyStateA(inputs), DeepPolyStateB(inputs),
                NeurifyState(inputs)
            ])
        return state
    ...
    def main(self, path, forced_active=None, forced_inactive=None):
        ...
        return self.run(forced_active=forced_active, forced_inactive=forced_inactive)

    def run(self, forced_active=None, forced_inactive=None):
        return self.interpreter().analyze(self.state(), outputs=self.outputs, forced_active=forced_active, forced_inactive=forced_inactive)
