...
class ForwardInterpreter(Interpreter):
        ...
        if self._log:
            ...
            if hasattr(state, "bounds") and isinstance(state.bounds, dict):
                ...
                if full:
                    ...
                    # mid_states = [f"  {k} -> {state.bounds[k]} | {state._domains[1].symbols.get(k, {})}" for k in state.bounds.keys() - state.inputs - outputs]
                    mid_states = [f"  {k} -> {state.bounds[k]} | {state.symbols.get(k, {})}" for k in state.bounds.keys() - state.inputs - outputs]
                    # mid_states = [f"  {k} -> {state.bounds[k]} | {state.poly[k]} | {state._poly[k]}" for k in state.bounds.keys() - state.inputs - outputs]
                    mid_states.sort()
                    print(mid_color + "\n".join(mid_states), Style.RESET_ALL)
                # outputs = [f"  {k} -> {state.bounds[k]}" for k in outputs]
                # outputs = [f"  {k} -> {state.bounds[k]} | {state._domains[1].symbols.get(k, {})}" for k in outputs]
                outputs = [f"  {k} -> {state.bounds[k]} | {state.symbols.get(k, {})}" for k in outputs]
                # outputs = [f"  {k} -> {state.bounds[k]} | {state.poly[k]} | {state._poly[k]}" for k in outputs]
                outputs.sort()
    ...
    def analyze(self, initial, earlystop=True, forced_active=None, forced_inactive=None, outputs=None):
        ...
        return activated, deactivated, found, (balanced, polarity, symbols)
