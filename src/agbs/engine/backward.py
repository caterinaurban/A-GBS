...
from agbs.abstract_domains.bias_domain import BiasState
from agbs.abstract_domains.state import State
from agbs.core.cfg import Node, Function, Activation
from agbs.core.expressions import BinaryComparisonOperation, Literal, VariableIdentifier, BinaryBooleanOperation
from agbs.engine.interpreter import Interpreter
from agbs.semantics.backward import DefaultBackwardSemantics
from agbs.abstract_domains.interval2_domain import Box2State
from agbs.abstract_domains.deeppoly_domain import DeepPolyState
from agbs.abstract_domains.neurify_domain import NeurifyState
from agbs.abstract_domains.symbolic3_domain import Symbolic3State
from agbs.abstract_domains.product_domain import ProductState
...
class BackwardInterpreter(Interpreter):
    ...
    def __init__(self, cfg, manager, domain, semantics, specification, steps=None, minL=None, startL=0.25, startU=2, maxU=None, cpu=None, precursory=None):
        ...
        from agbs.engine.bias_analysis import AbstractDomain
        ...
        self.splittable: List[VariableIdentifier] = None
    ...
    def feasibility(self, state, manager, disjuncts, key=None, chunk=None):
        ...
        for idx, value in enumerate(self.values):
            ...
            active, inactive, outcome, polarity = self.precursory.analyze(result, forced_active=f_active, forced_inactive=f_inactive, outputs=self.outputs)
            if polarity[0]:
        ...
        if len(outcomes) <= 1 and None not in outcomes:
            ...
            self.json['fair'] = curr
            lock.release()
            return True, list(), len(self.activations), None

        priority = min(polarities, key=lambda x: x[1]) if polarities else None
        if priority:
            names = list(map(str, self.splittable))
            combined = {key: abs(value) for key, value in priority[2][0].items() if key in names}
            for key, value in priority[2][1].items():
                if key in names:
                    combined[key] = combined[key] + abs(value)
            splitters = sorted(combined.items(), key=lambda x: x[1], reverse=True)
            splitters = [splitter[0] for splitter in splitters]
        else:
            splitters = None
        return feasible, patterns, _disjunctions, splitters
    ...
    def consumer(self, queue3, entry, manager):
        ...
        while True:
            ...
            for value in self.values:
                ...
                active, inactive, _, _ = self.precursory.analyze(result2, earlystop=False, outputs=self.outputs)
    ...
    def worker1(self, id, color, queue1, manager):
        ...
        """
        while True:
            # queue1.put((list(), (0, 0), self.startL, self.startU, 0, list(), list(ranges.items()), 0, list(self.splittable), 100, None))
            ...
            r_percent = '| Percent: {}, Level: {}'.format(percent, level)
            r_partition = '{} | {}'.format(r_assumptions, r_ranges, r_percent) if r_assumptions else '{}'.format(r_ranges, r_percent)
            print(color + r_partition + r_percent, Style.RESET_ALL)
            ...
            if feasible:    # the analysis is feasible
                ...
                if pivot1 < len(self.uncontroversial1):  # we still have to split the one-hot encoded
                    ...
                    if size == 0 and len(unpacked) > 1:  # unpack one-hots first if difference = 0
                    ...
                    elif self.splittable and splittable:
                        rangesdict = dict(ranges)
                        splitters = feasibility[3]

                        splitter = VariableIdentifier(splitters[0])
                        (lower, upper) = rangesdict[splitter]
                        ...
                        if splitters and size < upper - lower:
                            ...
                            queue1.put((assumptions, steps, size, disjuncts, pivot1, unpacked, _left, pivot2, splittable, _percent, None))
                            queue1.put((assumptions, steps, size, disjuncts, pivot1, unpacked, _right, pivot2, splittable, _percent, None))
                        else:
                            # queue1.put((assumptions, steps, size, disjuncts, pivot1, unpacked, ranges, pivot2, False, percent, None))
                            if 2 * self.minL <= size or disjuncts < self.maxU:  # autotuning is possible
                                (stepsL, stepsU) = steps
                                if stepsU < self.steps[1] and disjuncts < self.maxU:
                                    ...
                                    print(Fore.BLUE + "Upper bound increase from: {} to: {}".format(disjuncts,
                                                                                                    _disjuncts),
                                          Style.RESET_ALL)
                                elif stepsU < self.steps[1] and 2 * self.minL <= size:
                                    print("BB")
                                    _stepsL, _stepsU = stepsL, stepsU
                                    _size, _pivot2, _splittable = size / 2, 0, list(self.splittable)
                                    _disjuncts = disjuncts
                                    print(Fore.BLUE + "Lower bound decrease from: {} to: {}".format(size, _size),
                                          Style.RESET_ALL)
                                ...
                                print(Fore.BLUE + "Autotuned to: L = {}, U = {}".format(_size, _disjuncts),
                                      Style.RESET_ALL)
                                queue1.put((
                                           assumptions, (_stepsL, _stepsU), _size, _disjuncts, pivot1, unpacked, ranges,
                                           _pivot2, _splittable, percent, key))
                            else:
                                with self.explored.get_lock():
                                    self.explored.value += percent
                                    if self.explored.value >= 100:
                                ...
                                progress = 'Progress for #{}: {}% of {}% ({}% fair)'.format(id, self.feasible.value,
                                                                                            self.explored.value,
                                                                                            self.fair.value)
                                print(Fore.YELLOW + progress, Style.RESET_ALL)
    ...
    def analyze(self, initial, inputs=None, outputs=None, activations=None, analysis=True):
        ...
        with open(self.specification, 'r') as specification:
            ...
            if arity > 1:   # the sensitive feature is one-hot encoded
                ...
                _value1[self.sensitive[0]] = (-1, eval(pivot))       # OHLALA
            ...
            for uncontroversial in self.uncontroversial2:
                ranges[uncontroversial] = (-1, 1)        # OHLALA
        ...
        self.splittable = [uncontr for uncontr in self.uncontroversial2 if ranges[uncontr][0] < ranges[uncontr][1]]
        queue1.put((list(), (0, 0), self.startL, self.startU, 0, list(), list(ranges.items()), 0, list(self.splittable), 100, None))
